...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		// JDBC(Java DataBase Connectivity) : Java에서 DB에 연결할 수 있게 해주는 Java Programmiing API
		// (Java에 기본 내장된 인터페이스)
		...
		// 생성된 결과 (ResultSet, 성공한 행의 개수)를 반환(Java)
		...
		try {
			...
			// -> () 안에 작성된 클래스의 객체를 반환
			// -> 메모리에 객첵 생성되어지고 JDBC 필요 시 알아서 참조하여 사용
			...
			// 3. SQL 작성
			// ** JAVA 에서 작성되는 SQL은 마지막에 ; 을 찍으면 안된다!! **
			...
			// excuteQuery() : SELECT문 수행 메서드, ResultSet 반환
			
			// 3단계 : SQL을 수행해서 반환받은 결과 (ResultSet)를
			//		   한 행씩 접근해서 컬럼값 얻어오기
			
			while(rs.next()) {
				// rs.next() : rs가 참조하고 있는 RestultSet 객체의
